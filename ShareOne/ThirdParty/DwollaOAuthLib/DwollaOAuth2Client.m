...
//    NSString* url = [NSString stringWithFormat: @"https://www.dwolla.com/oauth/v2/token?client_id=%@&client_secret=%@&grant_type=authorization_code&redirect_uri=https://www.dwolla.com&code=%@", key, secret, code];
    ...
    NSString* url = [NSString stringWithFormat: @"https://uat.dwolla.com/oauth/v2/token?client_id=%@&client_secret=%@&grant_type=authorization_code&redirect_uri=http://dev.appmaisters.com/valuation/tests/saveform.php&code=%@", key, secret, code];
...
//    NSString* url = [NSString stringWithFormat:@"https://www.dwolla.com/oauth/v2/authenticate?client_id=%@&response_type=%@&redirect_uri=%@&scope=", keyParam, responseParam, redirectParam];
    ...
    NSString* url = [NSString stringWithFormat:@"https://uat.dwolla.com/oauth/v2/authenticate?client_id=%@&response_type=%@&redirect_uri=%@&scope=", keyParam, responseParam, redirectParam];
    ...
    NSLog(@"URL : %@", [returnURL.URL absoluteString]);
    return returnURL;
}
