...
//-------------------------------------------------------------------------------------------------
// description (for debugging), dumps form data
//-------------------------------------------------------------------------------------------------

- (NSString *)description
{
    NSData *body = [mpfURLRequest HTTPBody];
    if (mpfURLRequest != nil)
        return ([[[NSString alloc] initWithData:body encoding:NSASCIIStringEncoding] stringByReplacingOccurrencesOfString:@"\r" withString:@""]);
    else
        return @"";
}
