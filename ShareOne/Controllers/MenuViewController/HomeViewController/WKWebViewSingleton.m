    ...
    dispatch_once(&onceToken, ^{
        if (instance == nil)
        {
            instance = [[self alloc] init];
             instance.webviewSingle = [[WKWebView alloc] initWithFrame:CGRectMake(0, 65, [UIScreen mainScreen].bounds.size.width,[UIScreen mainScreen].bounds.size.height - [Configuration getGlobleDisablity])];
        }
    });
