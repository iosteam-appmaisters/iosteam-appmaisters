//
//  Services.h
//  THISWAY
//
//  Created by Sabika Batool on 2/4/16.
//  Copyright © 2016 Sabika Batool. All rights reserved.
//

#ifndef THISWAY_Services_h
#define THISWAY_Services_h

#define KWEB_SERVICE_BASE_URL                           @"http://dev.appmaisters.com/thisway/index.php/"

#define KWEB_SERVICE_GET_IMAGE_BASE_URL                 @"http://dev.appmaisters.com/thisway/"

#define KWEB_SERVICE_SIGNUP                             @"user/signup"

#define KWEB_SERVICE_LOGIN                              @"user/login"

#define KWEB_SERVICE_CHANGE_PASSWORD                    @"user/change_password"

#define KWEB_SERVICE_GET_STATES                         @"user/get_states"



#endif
